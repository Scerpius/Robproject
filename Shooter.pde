  ...
  void DisplayDamageTaken() {
    for ( int i = 0; i < bullets.size(); i++) {
      ...
      if (b.x + 27 >= posXEnemy && b.x < posXEnemy + 57 && b.y + 27 > posYEnemy && b.y < posYEnemy + 60) {
        display = true; // zet boolen display op waar zodat in de timer klass een timer kan afgaan die laat zien hoeveel damage jij doet voor halve seconden
      }
    }
    if (display) {
      textSize(32);
      text(player.dmg, posXEnemy, posYEnemy ); // laat de damage van speler zien op de locatie van speler
    }
  }
