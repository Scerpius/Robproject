class Player {
  ...
  void update() {
    ...
    if (start == true) {
      //visual of the shield
      if (shield) {
        ellipse(x + (w/2), y + (h / 2), w + 30, h + 20);
      }
      // gives dubbeldamage to player
      if (dubbelDamage == true && damageGiven == false) {
        dmg *= 2;
        damageGiven = true;
      }
      //returns damage to the value it was origanally
      if (dubbelDamage == false && damageGiven == true) {
        dmg /= 2;
        damageGiven = false;
      }
      // gives the player instakill damage
      if (instakill && instakillGiven == false) { 
        dmg *= 9999;
        instakillGiven = true;
      }
      // return the damage value from insta kill to normal
        if (instakill == false && instakillGiven) { 
        dmg /= 9999;
        instakillGiven = false;
      }
...
public class Player {
 ...
 pY = y;
 if (
 Collision(x +speedx, y, playerSprite.width, playerSprite.height) == false) {
 x = x+speedx;
 ;
 }
