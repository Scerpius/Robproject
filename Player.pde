class Player {
  ...
  void update() {
    ...
    if (start == true) {
      ...
      // if (collision.collision == false) {
      if ((keys[LEFT])||(keys['A'])) { //when you press the left arrow key or the A key then.....
        if (x >= objectList[0].x && x <= objectList[0].x + objectList[0].w &&y >= objectList[0].y && y <= objectList[0].y + objectList[0].h ) 
          // if (y < objectList[0].y + objectList[0].h - 50  && y  > objectList[0].y && x < objectList[0].x + objectList[0].w && x + 50 > objectList[0].x)
        { 
          playercolided = true;
        } else 
        {
