...
String databaseName = "zveenkec1"; 
String username = "veenkec1"; 
String password = "64B61p/oat7B68"; 
...
void draw() {
  ...
  if (state == 2) {
    ...
    if (player.hp <=0) {
      if(sql.connect()){
      sql.execute("INSERT INTO player_has_enemies(player_idplayer, enemies_enemyId, kills),VALUES ("+walkerKilled+")");
      sql.execute("INSERT INTO player_has_enemies(player_idplayer, enemies_enemyId, kills),VALUES ("+shooterKilled+")");
      }
      state = 4;
      //if (sql.connect()) {
